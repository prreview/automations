# Company: Voiceops
- **Description**: Voiceops turns calls, email, SMS, and chat into structured customer intelligence. The product ingests past interactions, maps products, funnels, intents, and risks, then pushes that signal into CRM, BI, and internal tools so sales, CX, product, and ops work from the same facts. The company says it serves multiple $1B+ revenue companies, including Fortune 500 names, and it raised a $5M seed round co-led by Bonfire Ventures and Twelve Below, with support from Village Global, Precursor, Not Boring Capital, and operators from Anthropic and Airbnb. Revenue looks like B2B SaaS with enterprise rollout work on top. Runway is not public, but the company is active, has 10 people on the team, and is still adding engineers.
- **Website**: https://voiceops.com
- **Founders/CTO**: Ethan Barhydt, Founder/CEO, LinkedIn: https://www.linkedin.com/pub/ethan-barhydt/a1/739/b35
- **Founder Email**: Not found
- **Founder LinkedIn**: https://www.linkedin.com/pub/ethan-barhydt/a1/739/b35
- **Hiring Signal**: true
- **Hiring sources**: https://news.ycombinator.com/item?id=48357725, https://www.ycombinator.com/companies/voiceops

## Hiring Signal Analysis
- **Open Roles Found**: Founding AI Engineer, Full-time, New York, NY, US, $200K-$250K base + 0.10% - 0.50% equity, 3+ years, AWS / Postgres / React / TypeScript / LLMs / AI Agents
- **Hiring Sources**:
  - https://news.ycombinator.com/item?id=48357725
  - https://www.ycombinator.com/companies/voiceops
  - https://www.voiceops.com/careers
- **Ways to Apply**: Cold Email / Cold Message on Twitter / Cold Message on LinkedIn

## Fit Check
- use this only when the stack lines up with my resume
- pick the matching resume slice: full stack, backend, ai, or frontend
- if the stack does not fit, skip the outreach

## Draft Outreach

### Cold Email
hi Ethan,

came across the founding ai engineer role at Voiceops and the stack looks close to the product and infra work i like.

i've spent 2.6+ years building product systems end to end, from a WaveMaker transpiler that turns proprietary markup into production next.js to a 4-person rava team where i owned backend, frontend, and deploys, and i keep my work here: github.com/yagyaraj234.

i also built trevyn, a pr reviewer that reads full repo context, and Voiceops feels like the kind of place where that mix would help.

if the stack is a fit, happy to chat.

Raj
yagyaraj.com | trevyn.dev

### Cold Message (LinkedIn / InMail)
hey Ethan,

saw the founding ai engineer opening at Voiceops and the stack looks close to the product and infra work i like.

i'm at wavemaker building a transpiler that turns proprietary markup into production next.js, and before that i was a founding engineer at rava on a 4-person team, owning the backend, frontend, and deploys; github.com/yagyaraj234.

i also built trevyn, a pr reviewer that reads full repo context, and i'd love to chat if you're open to it.

if the stack is a fit, happy to chat.

Raj
yagyaraj.com | trevyn.dev

### Twitter DM
hey ethan,

saw the founding ai engineer role at voiceops.

i'm at wavemaker building a transpiler into production next.js, and i was a founding engineer at rava on a 4-person team; github.com/yagyaraj234.

i built trevyn too, a pr reviewer that reads full repo context, and i think i'd fit well there.

if the stack is a fit, happy to chat.

Raj
yagyaraj.com | trevyn.dev
